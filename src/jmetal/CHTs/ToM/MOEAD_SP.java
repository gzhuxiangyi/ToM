...
public class MOEAD_SP extends Algorithm {
   ...
   */
  public MOEAD_SP(Problem problem) {
    super (problem) ;
...
//	  this.runTimes++;
    ...
    System.out.println("MOEAD-SP maxEvaluations = " +  maxEvaluations_);		
	System.out.println("MOEAD-SP popsize = " +  populationSize_ );		
	
	if (convergenceFlag == true) 
		  captureConvergence_ = new CaptureConvergence(problem_.getName(), "MOEADSP", problem_.getNumberOfObjectives());	
  ...
  void updateProblem(Solution indiv, int id, int type) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      /***** This part is new according to the violation of constraints *****/      
       double s1 = 0.1, s2 = 20;
       double tao = 0.0;
	   double max = -1.0E+10, min = 1.0E+10;

		// (it's time consuming)
		for (int j = 0; j < populationSize_; j++)
		{
			if (Math.abs(population_.get(j).getOverallConstraintViolation()) > max) { 
				max = Math.abs(population_.get(j).getOverallConstraintViolation()); 
			}
			
			if (Math.abs(population_.get(j).getOverallConstraintViolation()) < min) { 
				min = Math.abs(population_.get(j).getOverallConstraintViolation()); 
			}
		}
...
//		{
//			if (Population()[N(id)[i]].mConstraint>max){ max = Population()[N(id)[i]].mConstraint; }
//			if (Population()[N(id)[i]].mConstraint<min){ min = Population()[N(id)[i]].mConstraint; }
//		}
//		tao = min + 0.3*(max - min);

		// considering that if feasible solutions are directly better than infeasible solutions
		...
		//else if (!ind1.IsFeasible() && !ind2.IsFeasible())
//		{
//			f1 += ind1.mConstraint < tao ? (s1*ind1.mConstraint*ind1.mConstraint) : (s1*tao*tao + s2*(ind1.mConstraint - tao));
//			f2 += ind2.mConstraint < tao ? (s1*ind2.mConstraint*ind2.mConstraint) : (s1*tao*tao + s2*(ind2.mConstraint - tao));
//
//			if (f2 < f1) flag = true;
//		}
		
		double absCV1 = Math.abs(population_.get(k).getOverallConstraintViolation()) ;
		if (absCV1 < tao) {
			f1 = f1 + s1 * absCV1 * absCV1;
		} else {
			f1 = f1 + s1*tao*tao + s2* (absCV1 - tao) ;
		}

		double absCV2 = Math.abs(indiv.getOverallConstraintViolation());
		
		if ( absCV2 < tao) {
			f2 = f2 + s1 * absCV2 * absCV2;
		} else {
			f2 = f2 + s1*tao*tao + s2* (absCV2 - tao) ;
		}
