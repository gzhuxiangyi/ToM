...
public class CMOEADD extends Algorithm {
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		System.out.println("CMOEADD maxEvaluations = " +  maxEvaluations);		
		System.out.println("CMOEADD popsize = " +  populationSize_ );		
...
//		SqlUtils.CreateTable(problemName,methodName_);
		...
		for (int i = 0; i < populationSize_; i++) {
			if (population_.get(i).getOverallConstraintViolation() > -1e-6) {
				num_feasible++;
...
//			}
			...
			for (int i = 0; i < populationSize_; i++) {
				...
				if(crossover_.getClass().getSimpleName().equalsIgnoreCase("SBXCrossover")) {
					...
					Solution[] selectedParent =  matingSelection_constraint(cid, type);
					parents[0] = selectedParent[0];
					parents[1] = selectedParent[1];
					...
					System.out.print("unknown crossover operator");
				}
...
//		SqlUtils.InsertSolutionSet(problemName,population_);
	    ...
	    Ranking rk = new Ranking(finalSet);
	    return rk.getSubfront(0);
//		return population_;
	}
...
//		}
	...
	public void updateArchive(Solution indiv) {
		...
		if (indiv.getOverallConstraintViolation() > -1e-6) {	// indiv is feasible
