...
//	  this.runTimes++;
    ...
    System.out.println("MOEAD-SR maxEvaluations = " +  maxEvaluations_);		
	System.out.println("MOEAD-SR popsize = " +  populationSize_ );		
	
	if (convergenceFlag == true) 
		  captureConvergence_ = new CaptureConvergence(problem_.getName(), "MOEADSR", problem_.getNumberOfObjectives());	
...
//				System.out.print(newSolution.toString() + " \n");
  ...
  void updateProblem(Solution indiv, int id, int type) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if ( (isfeasible(population_.get(k)) && isfeasible(indiv)) || r < Pf ){
