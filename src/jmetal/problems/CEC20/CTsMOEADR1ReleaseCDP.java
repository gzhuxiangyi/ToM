/**
 * CTsMOEADR1ReleaseCDP.java 
 * 1. Developed based on CTsMOEADR1Release.
 * 2. Consider constraints in both stages
 * 3. Constrained are handled by CDP
 */
...
public class CTsMOEADR1ReleaseCDP extends Algorithm {
	 ...
	 */
	public CTsMOEADR1ReleaseCDP(Problem problem) {
		super(problem);		
	} // CTaMOEAD
...
//					  problem_.getNumberOfObjectives());	
		...
		System.out.println("=============Running CTs-MOEAR1Release CDP=============");
...
//			populationSize_ += 1;	
		...
		System.out.println("CTs-MOEAR1 Release CDP maxEvaluations = " +  maxEvaluations_);		
		System.out.println("CTs-MOEAR1 Release CDP popsize = " +  populationSize_ );		
...
//		initRandomWeight();		
		...
		do {		
			...
			if (!stage2_ && (evaluations_ >= 0.5 * this.maxEvaluations_|| unsuccessfulTrailsForFeasible_ > gap * populationSize_)) {	// 
				stage2_ = true;			
...
//			}
			...
			if (evaluations_ >= 0.9 * maxEvaluations_ && reset == false){		
				if (archive_.size() > 0) {				
...
//					}
	...
	public SolutionSet getInfeasibleSet(SolutionSet union) {
		...
		for (int i = 0;i < union.size();i++) {
        	if (Math.abs(union.get(i).getOverallConstraintViolation()) > 0.0) {
...
//		union_.sort((new FitnessComparator()));
		...
		for (int i = 0 ; i < union_.size();i++) { // for each solution
			...
			for(int j = 0; j < lambda_.length;j++) { // for each weight vector
...
//			maxNumber = Math.min(maxNumber, T_);
...
//					union_.get(closestList[k].get(0)).setRank(rank);		
				...
				if (closestList[k].size() >= numberOfConsidered) {			
					...
					ArrayList<Integer> minCVList = new ArrayList(); // Store solution index with minimal CV
					...
					for (int i = 0; i < closestList[k].size();i++) {
						Solution sol = union_.get(closestList[k].get(i));
						
						if (Math.abs(sol.getOverallConstraintViolation()) < minCV) {
							
							minCV = Math.abs(sol.getOverallConstraintViolation());
							minCVList.clear();
							minCVList.add(i);							
							
						} else if (Math.abs(sol.getOverallConstraintViolation()) == minCV) {
							minCVList.add(i);		
						}
