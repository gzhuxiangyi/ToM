...
public class Distance {
  ...
  public void crowdingDistanceAssignmentMin(SolutionSet solutionSet, int nObjs) {
	    int size = solutionSet.size();
	    ...
	    if (size == 1) {
	      solutionSet.get(0).setCrowdingDistance(0);
	      return;
	    } // if

	    if (size == 2) {
	      solutionSet.get(0).setCrowdingDistance(0);
	      solutionSet.get(1).setCrowdingDistance(0);
	      return;
	    } // if       
	    ...
	    for (int i = 0; i<nObjs; i++) {
	      ...
	      //Set de crowding distance            
	      front.get(0).setCrowdingDistance(0);
	      front.get(size-1).setCrowdingDistance(0);

	      for (int j = 1; j < size-1; j++) {
	        distance = front.get(j+1).getObjective(i) - front.get(j-1).getObjective(i);
	        distance = 1.0 - distance / (objetiveMaxn - objetiveMinn);
	        distance += front.get(j).getCrowdingDistance();
	        front.get(j).setCrowdingDistance(distance);
	      } // for
	      
	      for (int j = 0; j < size; j++) {	
		        front.get(j).setCrowdingDistance(front.get(j).getCrowdingDistance()/nObjs);
		  } // for
