...
public class EpsilonRanking {
	 ...
	 */
	public EpsilonRanking(SolutionSet solutionSet,double epsilon) {
		epsilon_ = epsilon;
		...
		for (int p = 0; p < (solutionSet_.size() - 1); p++) {
			...
			for (int q = p + 1; q < solutionSet_.size(); q++) {
				flagDominate = epsilonCompare(solutionSet.get(p),solutionSet.get(q));
	...
	public boolean isPseudoFeasible (Solution sol) {
		if (Math.abs(sol.getOverallConstraintViolation()) <= epsilon_) 
			return true;
		else
			return false;
	}
	
	public int epsilonCompare(Solution p,Solution q){
		double overall1, overall2;
	    overall1 = p.getOverallConstraintViolation();
	    overall2 = q.getOverallConstraintViolation();
	       
	    overall1 = Math.abs(overall1); // Take Abs 
	    overall2 = Math.abs(overall2);
	    
	    boolean isfeasible1 = isPseudoFeasible(p);
	    boolean isfeasible2 = isPseudoFeasible(q);
	    
	    if (isfeasible1 && !isfeasible2) {
	    	return -1;
	    } else if (!isfeasible1 && isfeasible2) {
	    	return 1;
	    } else if (!isfeasible1 && !isfeasible2) {// both are infeasible
    		  ...
    		  int flagDominate = dominance_.compare(p,q);
    		  return flagDominate;  
	    } // if 
	} // epsilonCompare 
